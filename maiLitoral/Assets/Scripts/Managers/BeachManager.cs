...
public class BeachManager : MonoBehaviour
{
    ...
    // Loads beaches, ranks and properties from the database.
    private void LoadBeachesFromDatabase(){
        ...
        DateTime firstDayOfLastMonth = firstDayOfThisMonth.AddMonths(-1);
         ...
         for (int i = 0; i < beachesFromDatabase.Count; i++){
            ...
            // Loads and displays the properties of a selected beach for a selected date.
    public void LoadBeachProperties(DateTime currentDate, int beachIndex) {
        ...
        if (reviewMode == false){
            ...
            propertiesText.text = "Facilities from " + date;
        }
    }
    // Opens the properties panel for the selected beach.
    ...
    // Updates a property when the user reviews it.
    ...
    // Enables review mode for the selected beach.
    ...
    // Returns from review mode to normal beach view.
